...
export function FeaturedStrip({ movie, label, ui }: FeaturedStripProps) {
  const stripLabel = label ?? ui.featuredStripDefaultLabel;
    ...
    <section aria-labelledby={`strip-${movie.id}`} className="glass-surface relative overflow-hidden rounded-2xl">
      <div className="relative min-h-[220px] sm:min-h-[260px] lg:min-h-[260px]">
        {imageSrc ? (
          <Image
            src={imageSrc}
            alt={ui.featuredStripImageAlt(movieTitle)}
            fill
            loading="lazy"
        ...
        <div className="relative z-10 flex h-full items-center px-5 py-6 sm:px-6 sm:py-8">
          <div className="max-w-xl space-y-2">
            <p className="text-[11px] uppercase tracking-[0.18em] text-zinc-300">{stripLabel}</p>
            <h3 id={`strip-${movie.id}`} className="text-xl font-semibold text-zinc-100 sm:text-2xl">
              {movieTitle}
            </h3>
            <p className="line-clamp-2 text-sm text-zinc-200">{movie.overview || ui.featuredStripOverviewFallback}</p>
            <Link
              href={detailHref}
